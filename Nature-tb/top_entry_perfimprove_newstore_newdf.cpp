...
#pragma HLS aggregate variable=Crows
	float C[Pa/P][16/T][P][T]; //32 is old Ta, 16 new. 
 ...
 //     #pragma HLS aggregate variable=tempB
	...
	for(int yy = 0; yy < co*Pa/P*Ta/T; yy++){
	#pragma HLS PIPELINE II = 1
		#pragma HLS dependence array variable=C inter false
		j = yy%(Ta/T);
		i = (yy/(Ta/T))%(Pa/P);
...
#pragma HLS aggregate variable=Inrows
	...
	float scratchpad[H][H][Ta]; //Cin=some number times Ta
...
void sstage1_3(blockvec_In_P *A, blockvec_W_T *B1, blockvec_W_T *B2, blockvec_W_T *B3, blockvec_W_T *B4,blockvec_W_T *B5,blockvec_W_T *B6,
	int it2,hls::stream<blockvec_In_P> &outpipe){
...
//#pragma HLS INTERFACE bram port=C storage_type=ram_2p
 ...
 // 	//  float bias2[L3];
	...
	CinBatchLoop: for (int it2=0;it2<Cin/Ta;it2++) { //output-channel-wise parallelism Cin/Ta:layer1=1->it2 can only be 0
		...
		sstage1_3(A, B1, B2, B3, B4,B5,B6,it2,outpipe);
		//================================End sstage1_3==========================================
